...
class VideoDataset_constrained_mask_tracking_sal_light_112(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[List[torch.Tensor], int]:
        ...
        frame_length = 1
        zero_mask_one = np.zeros((1,1,112,112), dtype=np.uint8)
        ...
        if load_mask==1:
            ...
            if 1:
                ...
                query_mask_array = np.zeros((frame_length,112,112),dtype=np.uint8)
                for i in range(frame_length):
                    ...
                    try:
                        query_mask = resize(query_mask, 112, 112)
                    except:
            ...
            #try:
                ...
                key_mask_array = np.zeros((frame_length,112,112),dtype=np.uint8)
                for i in range(frame_length):
                    cur_num = str(mask_frame_n-mid_n+i+1).zfill(5)
                    frame_name = 'image_'+cur_num
                    ...
                    try:
                        key_mask = resize(key_mask, 112, 112)
                    except:
